...
#include "Fl_My_Native_File_Chooser_common.cxx" // strnew/strfree/strapp/chrcat
...
// SET TYPE OF BROWSER
void Fl_My_Native_File_Chooser::type(int val)
{
    _btype = val;
}
...
// SET OPTIONS
void Fl_My_Native_File_Chooser::options(int val)
{
    _options = val;
}
...
void Fl_My_Native_File_Chooser::errmsg(const char *val)
{
    _errmsg = strfree(_errmsg);
    _errmsg = strnew(val);
}
...
void Fl_My_Native_File_Chooser::clear_pathnames()
{
    if (_pathnames) {
        while (--_tpathnames >= 0) {
...
void Fl_My_Native_File_Chooser::set_single_pathname(const char *s)
{
    clear_pathnames();
...
void Fl_My_Native_File_Chooser::add_pathname(const char *s)
{
    if (!_pathnames) {
...
void Fl_My_Native_File_Chooser::ClearOFN()
{
...
void Fl_My_Native_File_Chooser::ClearBINF()
{
...
void Fl_My_Native_File_Chooser::Win2Unix(char *s)
{
    for (; *s; s++)
        if (*s == '\\') {
            *s = '/';
...
void Fl_My_Native_File_Chooser::Unix2Win(char *s)
{
    for (; *s; s++) {
        if (*s == '/') {
            *s = '\\';
...
// SHOW FILE BROWSER
int Fl_My_Native_File_Chooser::showfile()
{
    ClearOFN();
    clear_pathnames();
    ...
    if (_parsedfilt != NULL) { // to convert a null-containing char string into a widechar string
        ...
        // NEW
        if (!_wpattern) {
            _wpattern = new WCHAR[FNFC_MAX_PATH];
        }
#else
        // OLD
...
static int CALLBACK Dir_CB(HWND win, UINT msg, LPARAM param, LPARAM data)
{
    ...
    case BFFM_INITIALIZED:
        if (data) {
            ::SendMessageW(win, BFFM_SETSELECTIONW, TRUE, data);
        }
        break;
...
int Fl_My_Native_File_Chooser::showdir()
{
    ...
    if (_btype == BROWSE_DIRECTORY) {
        _binf_ptr->ulFlags |= BIF_NONEWFOLDERBUTTON;
    }
    _binf_ptr->ulFlags |= BIF_USENEWUI | BIF_RETURNONLYFSDIRS;
#elif defined(BIF_USENEWUI) // Version 5.0
    if (_btype == BROWSE_DIRECTORY) {
        _binf_ptr->ulFlags |= BIF_EDITBOX;
    }
    else if (_btype == BROWSE_SAVE_DIRECTORY) {
        _binf_ptr->ulFlags |= BIF_USENEWUI;
    }
...
//
void Fl_My_Native_File_Chooser::title(const char *val)
{
    _title = strfree(_title);
    _title = strnew(val);
}
...
//
void Fl_My_Native_File_Chooser::filter(const char *val)
{
    _filter = strfree(_filter);
    clear_filters();
    if (val) {
...
// CLEAR FILTERS
void Fl_My_Native_File_Chooser::clear_filters()
{
    _nfilters = 0;
...
// ADD A FILTER
void Fl_My_Native_File_Chooser::add_filter(const char *name_in, // name of filter (optional: can be null)
...
//              Name     Wildcard
//
void Fl_My_Native_File_Chooser::parse_filter(const char *in)
{
    clear_filters();
    if (!in || in[0] == '\0') {
        return;
    ...
    char *wildprefix = new char[(slen + 1) * 2];
    wildprefix[0] = 0;
    char *comp = new char[(slen + 1) * 2];
    comp[0] = 0;
    char *name = new char[(slen + 1) * 2];
    name[0] = 0;
...
void Fl_My_Native_File_Chooser::preset_file(const char *val)
{
    _preset_file = strfree(_preset_file);
